...
namespace XBInsaat.Services.Dtos.Area
{
    ...
    public class NewsCreateDtoValidator : AbstractValidator<NewsCreateDto>
    {
        public NewsCreateDtoValidator()
        {
            ...
            RuleFor(x => x.WebsiteUrl).Empty().MinimumLength(3).WithMessage("Layihənin adının uzunluğu 3-dən az ola bilməz!").MaximumLength(200).WithMessage("Layihənin adının uzunluğu 3000-dən böyük ola bilməz!");
            RuleFor(x => x.Title).NotEmpty().WithMessage("Layihənin ad hissəsi boş olmamalıdır.").MinimumLength(3).WithMessage("Layihənin adının uzunluğu 3-dən az ola bilməz!").MaximumLength(3000).WithMessage("Layihənin adının uzunluğu 3000-dən böyük ola bilməz!");
